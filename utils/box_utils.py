...
def decode_fcos_boxes(box_preds, stride=4.0):
    ...
    # Bỏ F.relu để cho phép model dự đoán độ lệch âm (cần thiết vì CountYOLO dùng Hungarian Matching
    # có thể gán grid cell không nằm hẳn bên trong object, giống cơ chế của DETR)
    l = flat_boxes[:, 0, :]
    t = flat_boxes[:, 1, :]
    r = flat_boxes[:, 2, :]
    b = flat_boxes[:, 3, :]
